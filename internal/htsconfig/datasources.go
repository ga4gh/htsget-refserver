...
func (dataSource *DataSource) evaluatePath(id string) (string, error) {
	...
	if err != nil {
		log.Debugf("error in CreateRegexNamedParameterMap: %v", err)
		return "", err
	}
	parameterNamesMap, err := htsutils.CreateRegexNamedParameterMap("\\{(?P<paramName>.+?)\\}", dataSource.Path)
	if err != nil {
		log.Debugf("error in CreateRegexNamedParameterMap2: %v", err)
		return "", err
	}
...
func (registry *DataSourceRegistry) findFirstMatch(id string) (*DataSource, error) {
	for i := 0; i < len(registry.Sources); i++ {
		...
		if err != nil {
			log.Debugf("error in evaluatePatternMatch: %v", err)
			return nil, err
		}
		if match {
...
func (registry *DataSourceRegistry) GetMatchingPath(id string) (string, error) {
	...
	if matchingDataSource == nil || err != nil {
		log.Debugf("error in findFirstMatch or nil matchingDataSource: %v", err)
		return "", err
	}
	path, err := matchingDataSource.evaluatePath(id)
	if path == "" || err != nil {
		log.Debugf("error in evaluatePath or nil path: %v", err)
		return "", err
	}
	return path, err
}
